// CryptoJS AES OpenSSL-format decryption for GHL SSO blobs.
// Mirrors supabase/functions/decrypt-ghl-sso (kept in-sync).
...
export async function decryptGhlSso(b64: string, passphrase: string): Promise<any> {
  const raw = Uint8Array.from(atob(b64), (c) => c.charCodeAt(0));
  if (raw.length < 16 || String.fromCharCode(...raw.subarray(0, 8)) !== "Salted__") {
    throw new Error("Payload missing CryptoJS Salted__ prefix");
  }
