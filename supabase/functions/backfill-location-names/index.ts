...
// using the correct per-agency GHL Private Integration Token to look up
// /locations/{id}. Scope: optionally filter by ?companyId=<id> (defaults to all).
...
Deno.serve(async (req) => {
  ...
  const outcomes: Array<{
    location_id: string;
    status: "success" | "fallback_used" | "no_name_found" | "fetch_failed" | "missing_pit";
    source: string;
  ...
  // Concurrency pool. Resolve the PIT per-row using each row's ghl_company_id
  // so cross-agency workspaces each use their own agency's PIT.
  let cursor = 0;
  async function worker() {
    while (cursor < targets.length) {
      ...
      const { token: rowPit, secretName } = getGhlPit(row.ghl_company_id);
      if (!rowPit) {
        outcomes.push({ location_id: lid, status: "missing_pit", source: "missing", old_name: row.location_name, new_name: null, detail: `expected ${secretName}` });
        continue;
      }
      try {
        const res = await fetchAndResolveLocationName(lid, rowPit);
