...
// Copies every row of public.ghl_location_tokens from the source project to a
// destination project. Read-only against the source: never deletes or modifies
// source rows. Never overwrites destination rows (plain INSERT, no upsert).
// Never logs or returns token material or row contents.
...
Deno.serve(async (req) => {
  ...
  const { count: destination_count_before, error: destCountErr } = await dest
    .from("ghl_location_tokens")
    .select("id", { count: "exact", head: true });
  if (destCountErr) return j({ error: "destination_count_failed" }, 500);

  let copied = 0, skipped_existing = 0, failed = 0;
  let truncated = false;
  let offset = 0;
