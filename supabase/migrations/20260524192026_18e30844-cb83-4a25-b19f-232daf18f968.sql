...
ALTER TABLE public.title_companies
  ADD COLUMN IF NOT EXISTS entity_type text NOT NULL DEFAULT 'title_company';

ALTER TABLE public.archive_title_companies
  ADD COLUMN IF NOT EXISTS entity_type text NOT NULL DEFAULT 'title_company';

DO $$ BEGIN
  ALTER TABLE public.title_companies
    ADD CONSTRAINT title_companies_entity_type_chk
    CHECK (entity_type IN ('title_company','attorney'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.archive_title_companies
    ADD CONSTRAINT archive_title_companies_entity_type_chk
    CHECK (entity_type IN ('title_company','attorney'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
AS $function$
  ...
  SELECT
    ...
    r.deal_types, r.notes, r.entity_type, r.total_usage AS usage_count
  FROM ranked r
  WHERE r.rn = 1
