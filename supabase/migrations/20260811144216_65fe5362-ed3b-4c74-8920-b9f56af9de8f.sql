...
-- ---------------------------------------------------------------------------
-- Access: internal tables. Read-only for authenticated (gated to super_admin
-- by RLS); all writes go through SECURITY DEFINER functions / service_role.
-- ---------------------------------------------------------------------------
GRANT SELECT ON public.global_buyer_registry TO authenticated;
GRANT SELECT ON public.global_buyer_registry_sources TO authenticated;
GRANT ALL ON public.global_buyer_registry TO service_role;
GRANT ALL ON public.global_buyer_registry_sources TO service_role;
...
-- ---------------------------------------------------------------------------
-- Backfill: replay every existing buyer through the same upsert path.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
