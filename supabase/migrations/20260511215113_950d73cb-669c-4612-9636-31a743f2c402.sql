...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE m.ghl_location_id = NEW.ghl_location_id
      ...
      AND (
        NEW.ghl_pipeline_id IS NULL
        OR m.ghl_pipeline_id = NEW.ghl_pipeline_id
      )
  ) THEN
...
BEFORE INSERT OR UPDATE OF ghl_opportunity_id, ghl_location_id, ghl_pipeline_stage_id ON public.deals
FOR EACH ROW
