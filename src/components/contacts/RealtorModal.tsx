...
const empty = {
  ...
  brokerage: "", does_novations: false,
  markets: [] as string[], notes: "",
};

export function RealtorModal({
  open, onClose, onSaved, existing,
}: { open: boolean; onClose: () => void; onSaved: () => void; existing: RealtorRow | null }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (existing) {
      setForm({
        ...
        does_novations: !!existing.does_novations,
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload: any = {
      ...
      does_novations: form.does_novations,
      markets: form.markets,
      notes: form.notes.trim() || null,
    };
    const { error } = existing
      ? await (supabase as any).from("realtors").update(payload).eq("id", existing.id)
      : await (supabase as any).from("realtors").insert(withLocation(payload));
    setSaving(false);
    if (error) {
  ...
  async function remove() {
    ...
    const { error } = await (supabase as any).from("realtors").delete().eq("id", existing.id);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{existing ? "Edit Realtor" : "Add Realtor"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={save} className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <Input maxLength={160} value={form.brokerage} onChange={(e) => set("brokerage", e.target.value)} />
          </div>

          <div className="col-span-2 flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/20">
            <Checkbox id="novations" checked={form.does_novations} onCheckedChange={(v) => set("does_novations", !!v)} />
            <Label htmlFor="novations" className="cursor-pointer">Does Novations</Label>
          </div>
          ...
          <DialogFooter className="col-span-2 flex justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button type="submit" disabled={saving} className="bg-primary hover:bg-primary-hover">
                {saving ? "Saving…" : existing ? "Save Changes" : "Add Realtor"}
              </Button>
            </div>
          </DialogFooter>
        </form>
